...
function getSourceStatusAlerts(
  sources: SourceInventoryItem[],
): IntelligenceAlert[] {
  const alerts: IntelligenceAlert[] = [];

  for (const source of sources) {
    if (source.source === "Farcaster" && source.status === "Blocked") {
      alerts.push({
        id: "farcaster-blocked",
        severity: "warning",
        title: "Farcaster visibility blocked",
        detail:
          "Neynar cast/channel/trending APIs are blocked by the current plan, so tribe conversation evidence is unavailable.",
        source: "Neynar",
      });
    }

    if (source.source === "X/Twitter" && source.status === "Not Connected") {
      alerts.push({
        id: "x-not-connected",
        severity: "warning",
        title: "X / Twitter not connected",
        detail:
          "No X/Twitter connector is live, so any cross-source claim would only reflect GitHub + Dexscreener.",
        source: "X/Twitter",
      });
    }
  }
...
/**
 * Intelligence derivation (tribes, narratives, rabbit holes, bubble map,
 * opportunity radar, derived brief) is intentionally disabled.
 *
 * The previous deterministic cluster -> intelligence mapping shipped stop-word
 * "concepts" (of/on/in/is/to) with confident scores. Until the gated rebuild
 * (Phase 1) is reviewed and approved, this returns honest empty intelligence
 * plus real source inventory. No derived intelligence is fabricated here.
 */
export async function getIntelligenceDashboard(
  ...
  return {
    ...
    alerts: getSourceStatusAlerts(sourceInventory),
